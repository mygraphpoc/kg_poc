...
def run(question: str, on_step=None) -> dict:
    ...
    try:
        sql_raw = llm.invoke([HumanMessage(content=(
            ...
            f"9. Gold tables are already aggregated — do NOT add GROUP BY unless "
            f"you are aggregating further. If the table has one row per combination "
            f"(product+month, store+month etc.) just SELECT directly without GROUP BY.\n"
