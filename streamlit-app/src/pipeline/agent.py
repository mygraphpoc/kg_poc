    ...
    if structural:
        ...
        try:
            answer = llm.invoke([HumanMessage(content=(
                f"Answer the user question directly.\n""If the question asks to LIST or SHOW items (KPIs, tables, columns etc.) — return them as a bullet list, one item per line.\n""If the question asks for a count or summary — answer in 1-2 sentences.\n\n"
                f"{structural['text']}\n\nQuestion: {question}\nAnswer:"
            ))]).content.strip()
        except Exception:
