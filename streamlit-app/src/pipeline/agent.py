...
def run(question: str, on_step=None) -> dict:
    ...
    if structural:
        ...
        raw_rows = structural["rows"]
        raw_text = structural["text"]
        try:
            answer = llm.invoke([HumanMessage(content=(
                f"You are a data warehouse assistant. The user asked: {question}\n\n"
                f"Here is the data retrieved from the knowledge graph:\n{raw_text}\n\n"
                f"Instructions:\n"
                f"- The 'tableName' column contains the actual table names\n"
                f"- The 'platform' column is where the table lives (databricks or snowflake) — "
                f"this does NOT mean the table is not a Gold table\n"
                f"- The 'layer' column (if present) confirms the Gold/Silver/Bronze layer\n"
                f"- If asked to LIST tables or KPIs: return a bullet list grouped by platform\n"
                f"- Do NOT say 'no Gold tables exist' — all results shown ARE the Gold tables\n\n"
                f"Answer:"
            ))]).content.strip()
        except Exception:
            answer = raw_text
        return {"source":"GraphDB","sql":"","raw_table":raw_text,
                "answer":answer,"result_rows":raw_rows,
                "result_cols":list(raw_rows[0].keys()) if raw_rows else [],
    ...
    # ── 7. Generate SQL ───────────────────────────────────────────────────────
    # Build explicit column hint — tell LLM which columns satisfy the question intent
    # This prevents hallucination of generic names like "product" instead of "product_name"
    gold_col_names_set = {c.lower() for c, _ in gold_cols}

    INTENT_COLUMNS = [
        # (question keywords, preferred column if available in gold)
        (r"\bproduct[s]?\b",   ["product_name", "sku"]),
        (r"\bsku[s]?\b",        ["sku", "product_name"]),
        (r"\bcustomer[s]?\b",  ["full_name", "customer_name"]),
        (r"\bstore[s]?\b",     ["store_name"]),
        (r"\bemployee[s]?\b",  ["full_name", "employee_name"]),
        (r"\bsupplier[s]?\b",  ["supplier_name"]),
        (r"\bbrand[s]?\b",     ["brand"]),
        (r"\bcategory",        ["category", "sub_category"]),
        (r"\bmonth\b|\bmonthly\b", ["year_month", "month"]),
        (r"\byear\b|\byearly\b",   ["year", "year_month"]),
    ]

    column_hints = []
    q_lower_hint = question.lower()
    for pattern, preferred_cols in INTENT_COLUMNS:
        if re.search(pattern, q_lower_hint):
            available = [c for c in preferred_cols if c in gold_col_names_set]
            if available:
                column_hints.append(
                    f"  For '{pattern.replace(r'\\b','').replace('[s]?','')}' "
                    f"→ use column: {available[0]}"
                )

    hint_block = ""
    if column_hints:
        hint_block = (
            "\nCOLUMN MAPPING (use these exact column names for the intent):\n"
            + "\n".join(column_hints) + "\n"
        )
    ...
    try:
        sql_raw = llm.invoke([HumanMessage(content=(
            ...
            f"Question: {question}\n\n{schema_block}\n"
            f"{hint_block}\n"
            f"IMPORTANT: Gold tables are PRE-AGGREGATED — NEVER write GROUP BY or HAVING. Each row is already one aggregated record per grain. Just SELECT columns and ORDER BY.\n\nSTRICT RULES:\n"
            f"1. ONLY use column names from ACTUAL COLUMNS above. "
            f"Do NOT invent column names like 'product' or 'customer' — "
            f"use the exact names shown e.g. 'product_name', 'full_name'.\n"
            ...
            f"9. CRITICAL: Gold tables are PRE-AGGREGATED. NEVER add GROUP BY or HAVING. "
            f"Each row is already one aggregated record. Just SELECT and ORDER BY.\n"
            f"10. Return ONLY the SQL. No explanation. No markdown. No backticks.\n\nSQL:"
        ))]).content.strip()
