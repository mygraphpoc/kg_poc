...
def _dynamic_enrich(
    ...
    gold_fks = {c.lower() for c in gold_col_names if c.endswith("_sk") or c.endswith("_id")}

    # ── Step 1: Identify what logical entities the question needs ────────────
    # We use simple keyword detection for common DWH entities.
    # These are ENTITY CONCEPTS, not column names — they describe what the
    # user wants to see, not which table to use.
    ENTITY_SIGNALS = {
        # concept         : (possible column names in Silver dims)
        "product_detail": ["sku","product_name","brand","category","sub_category"],
        "customer_detail":["full_name","customer_name","email","first_name","last_name",
                           "segment","age_band","city","state_code"],
        "store_detail":   ["store_name","store_code","city","region","store_type"],
        "employee_detail":["full_name","employee_name","department","role","commission_rate"],
        "supplier_detail":["supplier_name","country","reliability_grade","lead_time_band"],
        "promotion_detail":["promo_name","promotion_name","promo_type","discount_pct"],
        "time_detail":    ["month","year_month","order_date","date","day","quarter"],
    }

    # Map entity signals to question keywords
    ENTITY_KEYWORDS = {
        "product_detail":  r"\b(sku|product\s*name|product\s*wise|by\s*product|per\s*product|"
                           r"brand|category|sub.?category|item\s*name|article)\b",
        "customer_detail": r"\b(customer\s*name|by\s*customer|per\s*customer|customer\s*wise|"
                           r"who\s*bought|buyer|client|segment|demographic)\b",
        "store_detail":    r"\b(store\s*name|by\s*store|per\s*store|store\s*wise|"
                           r"location|branch|outlet)\b",
        "employee_detail": r"\b(employee\s*name|by\s*employee|per\s*employee|sales\s*rep|"
                           r"agent|staff|commission\s*by)\b",
        "supplier_detail": r"\b(supplier\s*name|by\s*supplier|per\s*supplier|vendor\s*name)\b",
        "promotion_detail":r"\b(promo\s*name|promotion\s*name|by\s*promotion|campaign)\b",
        "time_detail":     r"\b(month|monthly|year|yearly|quarter|quarterly|daily|"
                           r"by\s*date|over\s*time|trend|period|year.?month|month.?year|"
                           r"time\s*series)\b",
    }
    ...
    for entity, pattern in ENTITY_KEYWORDS.items():
        if re.search(pattern, q_lower):
            needed_entities.append(entity)

    if not needed_entities:
        return {"needs_join": False, "join_plan": [], "log": "No additional dimensions needed"}

    # ── Step 2: Check which needed columns are already in Gold table ──────────
    missing_entities = []
    for entity in needed_entities:
        required_cols = ENTITY_SIGNALS[entity]
        # Does Gold already have at least one column for this entity?
        already_have = any(c.lower() in gold_col_names for c in required_cols)
        if not already_have:
    ...
    # ── Step 3: Fetch all Silver tables from GraphDB ──────────────────────────
    silver_tables = _get_all_silver_tables(token)

    # ── Step 4: For each missing entity, find the best Silver table ───────────
    join_plan = []
    used_tables = set()
    ...
    for entity in missing_entities:
        ...
        for silver_tname, silver_info in silver_tables.items():
            ...
            # Does this Silver table have any of the needed columns?
            matching_cols = [c for c in required_cols if c.lower() in silver_col_set]
            if not matching_cols:
                continue

            # Find a join key — look for Gold FK that matches a Silver PK/ID column
            join_col_silver = None
            join_col_gold   = None
            for gfk in gold_fks:
                # e.g. gold has product_sk → look for product_id in Silver
                base = gfk.replace("_sk","").replace("_id","").replace("_key","")
                for sfk in silver_col_set:
                    sfk_base = sfk.replace("_sk","").replace("_id","").replace("_key","")
                    if base == sfk_base or gfk == sfk:
                        join_col_silver = sfk
                        join_col_gold   = gfk
                        break
                if join_col_silver:
            ...
            if not join_col_silver:
                continue

            # Score this Silver table by how many needed cols it provides
            if len(matching_cols) > len(best_cols):
                best_table    = silver_tname
                best_cols     = matching_cols
                best_join_col = join_col_silver
                best_gold_fk  = join_col_gold
