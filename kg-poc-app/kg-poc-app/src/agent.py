...
def run(question: str) -> dict:
    ...
    # ── 6. Generate SQL ──────────────────────────────────────────────────────
    # Detect "all time / all months / no date filter" intent
    all_time = bool(re.search(
        r"\ball\s+(months?|time|years?|periods?|data|history|available)\b"
        r"|\bover\s+(all|entire|the\s+whole)\b"
        r"|\bno\s+date\s+filter\b"
        r"|\bhistorical\b"
        r"|\bevery\s+month\b",
        question, re.I))

    date_rule = (
        "3. DATE: NO date filter — return ALL rows (question asks for all months/all time)."
        if all_time else
        f"3. DATE: use subquery MAX for 'last quarter/month/year' — "
        f"e.g. WHERE year_quarter = (SELECT MAX(year_quarter) FROM {fqn}). "
        f"If question has NO time qualifier, do NOT add any date filter — return all rows."
    )
    ...
    sql_prompt = (
        ...
        f"7. Add LIMIT 100 unless the question asks for a total/summary.\n"
    ...
    ans_prompt = (
        ...
        f"Question: {question}\n"
        f"Table: {fqn}\n"
        f"SQL Result (first rows):\n{sql_result[:3000]}\n\n"
        f"Write a concise 2-4 sentence narrative summary of the key insight. "
        f"Mention the most important numbers. "
        f"Do NOT repeat the full table — a separate data table will be shown to the user.\n\nAnswer:"
    )
