...
def render() -> None:
    ...
    for item in st.session_state.get("history", []):
        ...
        ans   = item.get("answer", "").replace("\n", "<br/>")
        secs  = item.get("elapsed", "")
        table = src.split("→")[-1].strip() if "→" in src else ""
        err   = item.get("error", "")

        tbl_badge = f'<span class="badge bdg-tbl">📋 {table}</span>' if table else ""
        plat_label = plat.capitalize() or "GraphDB"
...
<div class="answer-card">
  ...
  <div class="meta-row">
    <span class="badge {badge_cls}">{ico} {plat_label}</span>
    {tbl_badge}
  </div>
  <div class="sec-lbl">Answer</div>
  <div class="ans-text">{"⚠️ " + err if err else ans or "—"}</div>
</div>
""", unsafe_allow_html=True)
