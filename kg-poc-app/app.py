...
# ── Env vars at module level (required by Databricks SDK) ─────────────────────
_h = _cfg("DATABRICKS_HOST")
os.environ["DATABRICKS_HOST"]  = f"https://{_h}" if _h and not _h.startswith("http") else _h
os.environ["DATABRICKS_TOKEN"] = _cfg("DATABRICKS_TOKEN")

# ── Session state ─────────────────────────────────────────────────────────────
for k, v in [("messages", []), ("pending", None),
...
# ── Tabs ──────────────────────────────────────────────────────────────────────
tab_chat, tab_test = st.tabs(["💬  Ask the DWH", "🧪  Test Suite"])
